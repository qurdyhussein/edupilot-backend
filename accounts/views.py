...
class AdminSignupView(APIView):
    ...
    def post(self, request):
        ...
        if not username or not email or not password:
            return Response({'message': 'Please provide username, email, and password.'},
                            status=status.HTTP_400_BAD_REQUEST)

        if User.objects.filter(email=email).exists():
            return Response({'message': 'Email address is already registered.'},
                            status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request):
        ...
        try:
            ...
            if not profile.reset_code or not profile.reset_code_created_at:
                return Response({'error': 'No reset code found'}, status=400)
